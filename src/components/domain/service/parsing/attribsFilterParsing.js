...
const QUERY_FIELDS = new Set(["name", "attr", "attrs", "neighbors", "source", "target"]);
...
export function normalizeQuerySyntax(input) {
  ...
  query = query.replace(
    /\b(name|attr|source|target)\s*:\s*("[^"]*"|[^\s(){} ,]+)/gi,
    (_, field, rawValue) => {
...
function validateQueryFields(query) {
  ...
  for (const match of unquotedQuery.matchAll(/\b([a-z][a-z0-9_-]*)\s*:/gi)) {
    ...
    if (!QUERY_FIELDS.has(field)) {
      throw new Error(`Unknown field '${field}'. Use name, attr, attrs, neighbors, source, or target.`);
    }
  }
}
...
function validateQueryFieldValues(query) {
  ...
  for (const match of maskedQuery.matchAll(/\b(name|attr|attrs|neighbors|source|target)\s*:/gi)) {
    const field = match[1].toLowerCase();
